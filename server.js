...
if (!JWT_SECRET) {
  console.warn(
    "WARNING: JWT_SECRET is not configured. Add JWT_SECRET to Railway Variables."
  );
}
...
app.use(
  cors({
    origin: (origin, callback) => {
      ...
      // such as direct browser requests or server-to-server requests.
      if (!origin) {
  ...
  (req, res) => {
    ...
    try {
      ...
      if (event.type === "customer.subscription.deleted") {
        ...
        const business = findBusinessByStripeCustomer(
...
/*
  ...
  IMPORTANT:
  This does not use passwords.
  It is suitable for testing but should be replaced
  with proper authentication before public launch.
*/
...
app.post(
  ...
  (req, res) => {
    try {
      ...
      const business = getOrCreateBusiness(
        normalizedEmail,
        businessName
      );
  ...
  async (req, res) => {
    try {
      ...
      if (!text?.trim()) {
        return res.status(400).json({
          error: "text_required",
          message: "Please enter a business transaction.",
        });
      }
      ...
      return res.status(500).json({
        error: "entry_failed",
        message: "Unable to record the transaction.",
      });
    }
  }
);
...
app.listen(
  ...
  () => {
    console.log(
      `AI Business Manager backend running on port ${PORT}`
    );
  }
);
